...
function adjustStep() {
    const MAX_STEP = 50;
    const MIN_STEP = 1;
    const TARGET_TIME_DIFF = 5000; // Target items within the last 5 seconds

    const timeSinceLastPublication = Date.now() - lastPublishedTime;

    // Reduce step if there are many consecutive errors
    if (consecutiveErrors > 5) {
        step = Math.max(Math.floor(step / 2), MIN_STEP);
        return;
    }

    // Increase step if items are too old
    if (timeSinceLastPublication > TARGET_TIME_DIFF) {
        step = Math.min(step + Math.floor(timeSinceLastPublication / 1000), MAX_STEP);
    }
    // Decrease step if items are very recent
    else if (timeSinceLastPublication < TARGET_TIME_DIFF / 2) {
        step = Math.max(step - 1, MIN_STEP);
    }
    // Maintain step within bounds
